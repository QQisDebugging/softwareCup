...
def main() -> None:
    ...
    seed_documents = loader.load_seed_documents(settings.seed_knowledge_paths)
    store.add_documents(seed_documents)
    first_chunk_count = store.chunk_count
    store.add_documents(seed_documents)
    assert store.chunk_count == first_chunk_count
    ...
    request = ResourceAgentRequest(
        ...
        documentTexts=[
            "Controller 层只负责 HTTP 请求参数接收、DTO 校验和响应封装，业务规则应放在 Service 层。",
        ],
    )
    store.add_documents(loader.load_generation_request_documents(request))
    ...
    assert "request.documentTexts" in response.content
