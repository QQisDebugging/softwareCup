...
class XfyunSparkProvider:
    ...
    def available(self) -> bool:
        return bool(self.settings.xfyun_api_key and self.settings.xfyun_api_secret)

    def complete(self, system_prompt: str, user_prompt: str) -> str:
        if not self.available:
            raise RuntimeError("XFYUN_API_KEY/XFYUN_API_SECRET are not configured.")
        payload = {
        ...
        headers = {
            "Authorization": f"Bearer {self.settings.xfyun_api_key}:{self.settings.xfyun_api_secret}",
...
class ProviderRouter:
    ...
    def complete(self, system_prompt: str, user_prompt: str) -> tuple[str, str, bool]:
        if self.settings.provider == "xfyun_spark":
            try:
                return self.spark.complete(system_prompt, user_prompt), self.spark.name, False
