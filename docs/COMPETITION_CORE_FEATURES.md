...
- 不只展示结果，还能展示先修诊断、资源策展、学习档案和智能体追踪，证明系统有完整学习闭环和可解释链路。
...
1. 创建学生画像和课程。
2. 运行先修诊断，展示准备度、诊断题和补救建议。
3. 运行资源策展，展示按画像重排后的资源包和覆盖图。
4. 针对同一课程问题发起智能辅导，展示引用、图解和学习动作。
5. 针对薄弱主题生成自适应测评并自动批改。
6. 展示画像维度被 `MASTERY_WEAKNESS` 和 `ERROR_PRONE_POINTS` 自动更新。
7. 生成学习档案报告，展示证据时间线、掌握度雷达、风险提示和教师评语草稿。
8. 打开智能体追踪，展示步骤、质量门禁、引用证据和降级事件。
