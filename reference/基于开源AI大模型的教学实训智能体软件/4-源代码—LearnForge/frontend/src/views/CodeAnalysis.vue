...
const languageModes = {
  Java: [java()], // Java mode needs to be added if necessary
  Python: [python()],
...
void DFT(Comp* f, int n, int rev) {
  ...
  for (int k = 0; k < n / 2;
    ...
    tmp[k] = g[k] + cur * h[k];
    // F(omega^{k+n/2}*n) = G(omega^k*{n/2}) - omega^k_n*H(omega^k_{n/2})
...
let chatList = ref([
  {
    role: "system",
    content:
      "你现在扮演一个资深代码助手,接下来请用富有教育意义的口吻和用户对话。使用markdown格式输出",
  },
]);

// // 更新会话列表的第一条内容
// const updateFirstChat = (value) => {
//   if (chatList.value.length > 0) {
//     chatList.value[0].content = ref([{"role":"system","content":"你现在扮演一个资深代码助手，用户给了如下代码+code+接下来请用代码助手的口吻和用户对话。"}]);
//   }
// };
  ...
  sparkWS.onopen = () => {
    chatList.value[0] = {
      ...
      content:
        "用户给的代码如下：" +
        code.value +
        "，你现在扮演一个资深代码助手，接下来请用富有教育意义的口吻和用户对话,先指出代码包含的知识点,使用markdown格式输出",
    };
...
const fallbackToHttpApi = async () => {
  console.log("使用HTTP API降级方案进行代码分析");
  ...
  try {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({ 
        query: `你是一个资深的代码专家，请分析以下${language}代码：\n\n\`\`\`${language}\n${codeContent}\n\`\`\`\n\n用户问题：${lastUserMessage}\n\n请用富有教育意义的口吻回答，指出代码包含的知识点，使用markdown格式输出。` 
      })
    });
    ...
    <div class="page-header">
      <div class="header-content">
        <div class="title-section">
          <v-icon color="primary" size="32" class="mr-3">mdi-file-tree</v-icon>
          <div>
            <h1 class="page-title">代码解析助手</h1>
            <p class="page-subtitle">深度解析代码逻辑，生成详细注释和知识点</p>
          </div>
        </div>
        <div class="stats-section">
          ...
          <div class="stat-item">
            <v-icon color="success">mdi-comment-text</v-icon>
            <span>注释生成</span>
          </div>
        </div>
      </div>
                  ...
                  <div ref="aiChatListRef" class="chat-list">
                    ...
                    <div class="welcome-message">
                      ...
                      <div class="welcome-content">
                        <div class="welcome-title">你好！我是AI代码解析助手</div>
...
.code-analysis-container::before {
  content: '';
  position: absolute;
  top: 0;
  left: 0;
