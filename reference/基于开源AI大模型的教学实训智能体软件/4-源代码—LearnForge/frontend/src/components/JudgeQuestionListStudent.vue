<!-- src/components/TrueFalseQuestionList.vue -->
<template>
    ...
    <div class="questions-section">
      <div 
        ...
        :class="{ 
          ...
          'correct': showAnswers && parseInt(selectedAnswers[index]) === parseInt(item.答案), 
          'incorrect': showAnswers && parseInt(selectedAnswers[index]) !== parseInt(item.答案)
        }"
      >
        <v-card class="question-card" elevation="0">
          <div class="question-header">
            ...
            <div class="question-status" v-if="showAnswers">
              <v-icon 
                :color="parseInt(selectedAnswers[index]) === parseInt(item.答案) ? 'success' : 'error'"
                size="24"
              >
                {{ parseInt(selectedAnswers[index]) === parseInt(item.答案) ? 'mdi-check-circle' : 'mdi-close-circle' }}
              </v-icon>
            </div>
          </div>
            ...
            <div class="options-container">
              ...
              >
                <div class="judge-options">
                  <div 
                    class="option-item true-option"
                    :class="{ 
                      'selected': selectedAnswers[index] === 0,
                      'correct': showAnswers && item.答案 === '0',
                      'incorrect': showAnswers && selectedAnswers[index] === 0 && item.答案 !== '0'
                    }"
                  >
                    <v-radio
                      label="正确"
                    ...
                    <div class="option-icon">
                      <v-icon color="success" size="24">mdi-check-bold</v-icon>
                    </div>
                  </div>
                  <div 
                    class="option-item false-option"
                    :class="{ 
                      'selected': selectedAnswers[index] === 1,
                      'correct': showAnswers && item.答案 === '1',
                      'incorrect': showAnswers && selectedAnswers[index] === 1 && item.答案 !== '1'
                    }"
                  >
                    <v-radio
                      label="错误"
                    ...
                    />
                    <div class="option-icon">
                      <v-icon color="error" size="24">mdi-close-thick</v-icon>
                    </div>
                  </div>
                </div>
              </v-radio-group>
...
export default {
  ...
  methods: {
    async submit() {
      ...
      try {
        await axios.post(`${this.$backendUrl}/submitContestAnswers`, data);
        pushRequestTimes(1);
      } catch (error) {
    ...
    calculateCorrectAnswers() {
      ...
      for (const [index, selected] of Object.entries(this.selectedAnswers)) {
        if (parseInt(selected) === parseInt(this.parsedQuestions[index].答案)) {
          correct++;
        }
      }
      return correct;
